...
void print_tie();
int get_board_size();
void setup_new_board(char boards[][N] , int n);
int doing_move_step(int player_index, char board[][N], int n);
int verify_right_step(char board[][N], int n, int i, int j);
bool check_winner(int player_index, char board[][N], int n);
...
int main()
{
    ...
    char board[N*N][N] = { 0 };
    setup_new_board(board, n);
    print_board(&board[0], n);
    int step_num = 0;
    while (step_num < n * n)
    {
        int player_index = step_to_player_inx(step_num);
        step_num += doing_move_step(player_index, &board[step_num*N], n);
        print_board(&board[step_num*N], n);
        if (check_winner(player_index, &board[step_num*N], n)) {
            print_winner(player_index + 1);
            break;
        }
    }
    if(check_winner(step_to_player_inx(step_num + 1), &board[step_num*N], n))
        print_tie();
}
...
int doing_move_step(int player_index, char board[][N], int n)
{
    ...
    while (true)
    {
        ...
        while (scanf("%d", &col) < 1)
            print_error();
        if(verify_right_step(board, n, row_or_undo - 1, col - 1) != 0)
        {
            row_or_undo = col = -1;
            print_error();
        }
...
bool check_winner(int player_index, char board[N][N], int n)
{
    ...
    int full_line = n, cnt_major_diagonal = 0, cnt_minor_diagonal = 0;
    for (int i = 0; i < n ; i++)
    {
        if(board[i][i] == p_sign)
            cnt_major_diagonal++;
        if(board[i][n-1-i] == p_sign)
            cnt_minor_diagonal++;
